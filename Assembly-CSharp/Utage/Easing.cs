...
namespace Utage
{
	public static class Easing
	{
		...
		{
			return GetCurve(0f, 1f, value, type);
		}
		...
		{
			return 1f - Mathf.Cos(value * ((float)Math.PI / 2f));
		}

		public static float EaseOutSine(float value)
		{
			return Mathf.Sin(value * ((float)Math.PI / 2f));
		}

		public static float EaseInOutSine(float value)
		{
			return -0.5f * (Mathf.Cos((float)Math.PI * value) - 1f);
		}
